        ...
        if (params.fileParamsMap.isEmpty() && !isMultipart) {
            ...
            for (Map.Entry<String, List<HttpParams.FileWrapper>> entry : params.fileParamsMap.entrySet()) {
                ...
                for (HttpParams.FileWrapper fileWrapper : fileValues) {
                    ...
                    try {
                        String fileName = URLEncoder.encode(fileWrapper.fileName.substring(0, fileWrapper.fileName.lastIndexOf(".")), "utf-8");
                        fileName += fileWrapper.fileName.substring(fileWrapper.fileName.lastIndexOf("."));
                        multipartBodybuilder.addFormDataPart(entry.getKey(), fileName, fileBody);
                    } catch (UnsupportedEncodingException e) {
                        e.printStackTrace();
                    }
                }
